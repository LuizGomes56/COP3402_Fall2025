...
static char *read_file_ascii_filtered(char *path) {
...
static char *remove_block_comments(char *s) {
    int n = strlen(s);
...
static bool is_all_digits(char *s) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        // get over the Box<dyn Display> usage (dynamic dispatch);
        char *special = NULL;
